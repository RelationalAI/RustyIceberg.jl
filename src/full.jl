...
"""
    next_tasks(task_stream::FileScanTaskStream, count::Integer=0) -> Vector{FileScanTaskHandle}

Pull up to `count` file scan tasks from the stream. `count=0` drains all remaining tasks.
Returns an empty vector when the stream is exhausted.
"""
function next_tasks(task_stream::FileScanTaskStream, count::Integer=0)
    response = TaskBatchResponse()
    async_ccall(response) do handle
        @ccall rust_lib.iceberg_next_file_scan_tasks(
            task_stream.ptr::Ptr{Cvoid},
            count::Csize_t,
    ...
    @throw_on_error(response, "iceberg_next_file_scan_tasks", IcebergException)

    if response.tasks == C_NULL || response.count == 0
        return FileScanTaskHandle[]
    end

    tasks = [FileScanTaskHandle(unsafe_load(response.tasks, i)) for i in 1:response.count]
    free_task_ptr_array(response.tasks, response.count)
    return tasks
end

"""
    read_tasks(reader::ArrowReaderContext, tasks::Vector{FileScanTaskHandle})::ArrowStream

Read multiple file scan tasks into a single Arrow stream. **Consumes all tasks.**
"""
function read_tasks(reader::ArrowReaderContext, tasks::Vector{FileScanTaskHandle})
    ptrs = Ptr{Cvoid}[task.ptr for task in tasks]
    response = ArrowStreamResponse()
    GC.@preserve ptrs async_ccall(response) do handle
        @ccall rust_lib.iceberg_read_file_scan_tasks(
            reader.ptr::Ptr{Cvoid},
    ...
    @throw_on_error(response, "iceberg_read_file_scan_tasks", IcebergException)
    return response.value
end
