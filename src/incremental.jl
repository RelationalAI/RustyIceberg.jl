...
"""
    next_append_tasks(streams::IncrementalFileScanTaskStreams, count::Integer=0) -> Vector{AppendTaskHandle}

Pull up to `count` append tasks from the stream. `count=0` drains all remaining tasks.
Returns an empty vector when the stream is exhausted.
"""
function next_append_tasks(streams::IncrementalFileScanTaskStreams, count::Integer=0)
    response = TaskBatchResponse()
    async_ccall(response) do handle
        @ccall rust_lib.iceberg_next_append_tasks(
            streams.ptr::Ptr{Cvoid},
            count::Csize_t,
            response::Ref{TaskBatchResponse},
            handle::Ptr{Cvoid}
        )::Cint
    end
    @throw_on_error(response, "iceberg_next_append_tasks", IcebergException)

    if response.tasks == C_NULL || response.count == 0
        return AppendTaskHandle[]
    end

    tasks = [AppendTaskHandle(unsafe_load(response.tasks, i)) for i in 1:response.count]
    free_task_ptr_array(response.tasks, response.count)
    return tasks
end

"""
    next_delete_tasks(streams::IncrementalFileScanTaskStreams, count::Integer=0) -> Vector{DeleteTaskHandle}

Pull up to `count` delete tasks from the stream. `count=0` drains all remaining tasks.
Returns an empty vector when the stream is exhausted.
"""
function next_delete_tasks(streams::IncrementalFileScanTaskStreams, count::Integer=0)
    response = TaskBatchResponse()
    async_ccall(response) do handle
        @ccall rust_lib.iceberg_next_delete_tasks(
            streams.ptr::Ptr{Cvoid},
            count::Csize_t,
            response::Ref{TaskBatchResponse},
            handle::Ptr{Cvoid}
    ...
    @throw_on_error(response, "iceberg_next_delete_tasks", IcebergException)

    if response.tasks == C_NULL || response.count == 0
        return DeleteTaskHandle[]
    end

    tasks = [DeleteTaskHandle(unsafe_load(response.tasks, i)) for i in 1:response.count]
    free_task_ptr_array(response.tasks, response.count)
    return tasks
end

"""
    read_append_tasks(reader::ArrowReaderContext, tasks::Vector{AppendTaskHandle})::ArrowStream

Read multiple append tasks into a single Arrow stream. **Consumes all tasks.**
"""
function read_append_tasks(reader::ArrowReaderContext, tasks::Vector{AppendTaskHandle})
    ptrs = Ptr{Cvoid}[task.ptr for task in tasks]
    response = ArrowStreamResponse()
    GC.@preserve ptrs async_ccall(response) do handle
        @ccall rust_lib.iceberg_read_append_tasks(
            reader.ptr::Ptr{Cvoid},
            pointer(ptrs)::Ptr{Ptr{Cvoid}},
            length(ptrs)::Csize_t,
            response::Ref{ArrowStreamResponse},
            handle::Ptr{Cvoid}
        )::Cint
    end
    @throw_on_error(response, "iceberg_read_append_tasks", IcebergException)
    return response.value
end

"""
    read_delete_tasks(reader::ArrowReaderContext, tasks::Vector{DeleteTaskHandle})::ArrowStream

Read multiple delete tasks into a single Arrow stream. **Consumes all tasks.**
"""
function read_delete_tasks(reader::ArrowReaderContext, tasks::Vector{DeleteTaskHandle})
    ptrs = Ptr{Cvoid}[task.ptr for task in tasks]
    response = ArrowStreamResponse()
    GC.@preserve ptrs async_ccall(response) do handle
        @ccall rust_lib.iceberg_read_delete_tasks(
            reader.ptr::Ptr{Cvoid},
            pointer(ptrs)::Ptr{Ptr{Cvoid}},
            length(ptrs)::Csize_t,
            response::Ref{ArrowStreamResponse},
    ...
    @throw_on_error(response, "iceberg_read_delete_tasks", IcebergException)
    return response.value
end
