...
use crate::error_codes::{classified_error, classify, STATE_RESOURCE_FREED, STATE_WRITER_CLOSED};
...
export_runtime_op!(
    ...
    || {
        if table.is_null() {
            return Err(classified_error(STATE_RESOURCE_FREED, "Resource has been freed", "Null table pointer provided"));
        }
        if parquet_props.is_null() {
            return Err(classified_error(STATE_RESOURCE_FREED, "Resource has been freed", "Null parquet_props pointer provided"));
        }
...
export_runtime_op!(
    ...
    || {
        if writer.is_null() {
            return Err(classified_error(STATE_RESOURCE_FREED, "Resource has been freed", "Null writer pointer provided"));
        }
    ...
    async {
        ...
        let mut concrete = state
            ...
            .unwrap()
            .take()
            .ok_or_else(|| classified_error(STATE_WRITER_CLOSED, "Writer has already been closed", "Writer already closed"))?;
