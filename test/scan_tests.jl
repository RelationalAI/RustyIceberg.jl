...
@testset "Batch API - full scan" begin
    snapshot_path = "s3://warehouse/tpch.sf01/customer/metadata/00001-76f6e7e4-b34f-492f-b6a1-cc9f8c8f4975.metadata.json"
    ...
    # Pull all tasks at once (count=0)
    tasks = RustyIceberg.next_tasks(task_stream)
    @test tasks isa Vector{RustyIceberg.FileScanTaskHandle}
    @test length(tasks) > 0

    # Verify stream is exhausted
    more = RustyIceberg.next_tasks(task_stream)
    @test isempty(more)

    # Read all tasks at once
    stream = RustyIceberg.read_tasks(reader, tasks)
    @test stream != C_NULL
    ...
    RustyIceberg.free_stream(stream)
    @test total_rows > 0
    println("✅ Batch API (full): $(length(tasks)) tasks, $total_rows rows")
...
@testset "Batch API - incremental scan" begin
    snapshot_path = "s3://warehouse/tpch.sf01/customer/metadata/00001-76f6e7e4-b34f-492f-b6a1-cc9f8c8f4975.metadata.json"
    ...
    # Pull all append tasks at once
    append_tasks = RustyIceberg.next_append_tasks(task_streams)
    @test append_tasks isa Vector{RustyIceberg.AppendTaskHandle}

    # Pull all delete tasks at once
    delete_tasks = RustyIceberg.next_delete_tasks(task_streams)
    @test delete_tasks isa Vector{RustyIceberg.DeleteTaskHandle}

    # Read append tasks if any
    append_rows = 0
    if !isempty(append_tasks)
        stream = RustyIceberg.read_append_tasks(reader, append_tasks)
        while true
            bp = RustyIceberg.next_batch(stream)
            if bp == C_NULL; break; end
            batch = unsafe_load(bp)
            arrow_table = Arrow.Table(unsafe_wrap(Array, batch.data, batch.length))
            append_rows += nrow(DataFrame(arrow_table))
            RustyIceberg.free_batch(bp)
        end
        RustyIceberg.free_stream(stream)
    end

    # Read delete tasks if any
    delete_rows = 0
    if !isempty(delete_tasks)
        stream = RustyIceberg.read_delete_tasks(reader, delete_tasks)
        while true
            bp = RustyIceberg.next_batch(stream)
            if bp == C_NULL; break; end
            batch = unsafe_load(bp)
            arrow_table = Arrow.Table(unsafe_wrap(Array, batch.data, batch.length))
            delete_rows += nrow(DataFrame(arrow_table))
            RustyIceberg.free_batch(bp)
        end
        RustyIceberg.free_stream(stream)
    end

    println("✅ Batch API (incremental): $(length(append_tasks)) appends ($append_rows rows), $(length(delete_tasks)) deletes ($delete_rows rows)")
